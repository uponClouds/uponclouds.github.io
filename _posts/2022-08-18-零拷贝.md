...
![](https://blogimg-1253107768.cos.ap-guangzhou.myqcloud.com/blogImage/20220714222033.png)
...
从流程图可以看出，**传统IO的读写流程**，包括了4次上下文切换（4次用户态和内核态的切换），4次数据拷贝（**两次CPU拷贝以及两次的DMA拷贝**)，那什么是DMA拷贝呢？
...
![](https://blogimg-1253107768.cos.ap-guangzhou.myqcloud.com/blogImage/20220714222448.png)
...
正是**多个虚拟内存可以指向同一个物理地址**，可以把内核空间和用户空间的虚拟地址映射到同一个物理地址，这样的话，就可以减少IO的数据拷贝次数啦，示意图如下![](https://blogimg-1253107768.cos.ap-guangzhou.myqcloud.com/blogImage/20220714222532.png)

### DMA技术

DMA，英文全称是 **Direct Memory Access**，即直接内存访问。**DMA **本质上是一块主板上独立的芯片，允许外设设备和内存存储器之间直接进行 IO 数据传输，其过程**不需要CPU的参与**。

我们一起来看下 IO 流程，DMA 帮忙做了什么事情:![](https://blogimg-1253107768.cos.ap-guangzhou.myqcloud.com/blogImage/20220714222612.png)
...
`mmap+write`实现的零拷贝流程如下：![](https://blogimg-1253107768.cos.ap-guangzhou.myqcloud.com/blogImage/20220714222812.png)
...
sendfile实现的零拷贝流程如下：![sendfile实现的零拷贝](https://blogimg-1253107768.cos.ap-guangzhou.myqcloud.com/blogImage/20220714222908.png)
...
sendfile+DMA scatter/gather实现的零拷贝流程如下：![](https://blogimg-1253107768.cos.ap-guangzhou.myqcloud.com/blogImage/20220714223052.png)
